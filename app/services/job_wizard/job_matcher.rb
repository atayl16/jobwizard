...
module JobWizard
  ...
  class JobMatcher
    ...
    def calculate_match_score(job)
      ...
      return 0 unless job_text.include?('ruby on rails') || job_text.include?('rails')
      ...
      secondary_skills = %w[mysql redis aws kubernetes sidekiq html css]
      ...
      fullstack_terms = %w[full-stack fullstack frontend backend api]
    ...
    def match_reasons(job)
      ...
      # Check for Rails
      reasons << 'Ruby on Rails position' if job_text.include?('ruby on rails') || job_text.include?('rails')

      # Check primary skills
      primary_skills = %w[react javascript js]
      matched_primary = primary_skills.select { |skill| job_text.include?(skill) }
      reasons << "Uses your skills: #{matched_primary.join(', ')}" if matched_primary.any?

      # Check secondary skills
      secondary_skills = %w[mysql redis aws kubernetes sidekiq]
      matched_secondary = secondary_skills.select { |skill| job_text.include?(skill) }
      reasons << "Familiar tech: #{matched_secondary.join(', ')}" if matched_secondary.any?

      # Check if it's remote
      reasons << '100% remote position' if job.remote?

      # Check for seniority level
      senior_terms = %w[senior lead principal staff]
      reasons << 'Matches your experience level' if senior_terms.any? { |term| job_text.include?(term) }
    ...
    def matching_stats
...

      {
        ...
        match_percentage: total_jobs.positive? ? (matching_jobs_count.to_f / total_jobs * 100).round(1) : 0
      }
    end
  end
end
